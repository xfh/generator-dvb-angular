...
exports.injectRoute = function (moduleFile, uirouter, name, ctrl, ctrlAs, route, routeUrl, title, that) {
    ...
    if (uirouter) {
        ...
        var routeFile = moduleFile.replace('module', 'route');
        exports.addToFile(routeFile, code, exports.STATE_MARKER);
    } else {
...
exports.askForModule = function (type, that, cb) {
    ...
    // remove primary application module from selection (cannot add stuff to it)
    modules.splice(0, 1);
    ...
    choices[0] += ' (Application Core Module)';
